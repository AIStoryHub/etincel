...
Published on npm as `etincel` (current version 0.9.5). `npx etincel serve`
resolves and runs it with no local build step.

## Installation steps

1. **Add the server to the MCP client's config**, no clone or build needed:
   ...
   - Claude Code: `/plugin marketplace add AIStoryHub/etincel` then
...
2. **Restart the MCP client** so it spawns the new server process.

3. **Verify the install** by calling the `list_styles` tool. Expect a
...
Only needed for development work on the server itself, not for installing
it for a user:
...
Then get the absolute path to the built entry point (`realpath
dist/server.js` — a relative path will not resolve once the MCP client
spawns the process from its own working directory) and point the client at
it directly:
...
- **Tools appear but `list_styles`/`audit_text` return empty or error on
  data lookups, when running from a local clone** — `dist/data/*.json` is
  missing. `npm run build`'s `cp` step didn't run; re-run `npm run build`
  (not just `tsc` directly). This doesn't apply to the `npx etincel serve`
  path, which ships the data bundled in the published package.
...
- **Client shows the server as configured but tools never appear** — the
  client wasn't restarted after step 1, or the config JSON has a syntax
  error (trailing comma, wrong nesting). Re-validate the JSON.
- **Assuming an API key is required.** None is. If a setup flow is asking
  for one, something has gone wrong upstream in that flow, not in this
  server.
